...
class Cage:
    ...
    def interaction(self, *cell_list):
        ...
        if (self.material.name is 'Water') and self.fraction >= 0.8:  # check if saved_q >= lambda * effective_mass
        ...
        if (self.material.name is 'Ice') and self.fraction >= 0.8:  # check if saved_q >= lambda * effective_mass
            self.checker_transformation = False  # exit if False
            self.fraction = 0  # change to initial
        ...
        if round(self.temperature, 1) == self.material.trans_temperature and self.checker_transformation:
            self.fraction += self.upd_q / (self.material.fusion * self.mass)
            self.upd_q = 0
